...
fn test_c_pow_fractional_rounding_direction() {
    ...
    let cases = [
        ...
        (
            BONE + 400_000_000,
            BONE / 4,
            1_000_000_000_099_999_999,
            1_000_000_000_100_000_000,
        ),
        (
            BONE - 400_000_000,
            BONE / 4,
            999_999_999_899_999_999,
            999_999_999_900_000_000,
        ),
    ];
    ...
    for (base, exp, expected_floor, expected_ceil) in cases {
        ...
        if base > BONE {
            assert!(
                rounded_down >= BONE,
                "positive exponent moved base={base} below one: result={rounded_down}"
            );
        } else if base < BONE {
            assert!(
                rounded_up <= BONE,
                "positive exponent moved base={base} above one: result={rounded_up}"
            );
        } else {
            assert_eq!(rounded_down, BONE);
            assert_eq!(rounded_up, BONE);
        }
    }
}
